...
VALIDATION_PROMPTS = [
    "The Founding Fathers of America, including George Washington and Benjamin Franklin, are gathered around a table, intensely discussing the Declaration of Independence in a candle-lit room, adorned with flags of the thirteen colonies",
    "European kings from various dynasties, wearing crowns and royal robes, are seated together in a grand, ornately decorated palace hall, discussing alliances and treaties over a map of Europe.",
    "A security guard patrolling in a hall way",
    "A doctor examining test results on a computer",
    # "European kings from various dynasties, wearing crowns and royal robes, are seated together in a grand, ornately decorated palace hall, discussing alliances and treaties over a map of Europe.",
    # "The Emperor of China, dressed in traditional imperial robes with dragon embroidery, sits on the Dragon Throne in the Forbidden City, surrounded by court officials and guards, under a canopy of silk.",
]

prefix = 'best_quality,4k'

VALIDATION_PROMPTS = list([prefix + x for x in VALIDATION_PROMPTS])
# VALIDATION_PROMPTS = [
...
def log_validation(args, unet, vae, accelerator, weight_dtype, epoch, aesthetic_predictor, global_step, is_final_validation=False,reference_model=None,fixed_noise=None):
    ...
    if aesthetic_predictor is not None:
        aesthetic_predictor.to(accelerator.device)
        scores = [aesthetic_predictor(prompt,image) for prompt,image in zip(VALIDATION_PROMPTS,images)]
        if len(scores) > 0:
            scores_mean = {
                k:np.mean([score[k] for score in scores]) for k in scores[0].keys()
            }
            print('----eval-----')
            print(scores_mean)
        else:
            scores_mean = {}
        aesthetic_predictor.to('cpu')
    else:
        scores_mean = {}
    ...
    if accelerator.is_main_process:
        for tracker in accelerator.trackers:
            ...
            if tracker.name == "wandb":
                accelerator.log((scores_mean), step=global_step-1)
                tracker.log(
                    {
                        tracker_key: [
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--dataloader",
        type=str,
        choices=['default','laion','pick','diversity','dpo'],
        default='laion',
        help=("dataloader"),
    )
    parser.add_argument(
    ...
    for idx,caption in enumerate(examples[caption_key]):
        if random.random() < args.proportion_empty_prompts:
            ...
            if args.loss == 'cft':
                ...
                assert label in [0,1]
                prefix = '[good image] ' if label == 1 else '[bad image]'
...
def make_weights_for_balanced_classes(dataset):  
    ...
    for item in tqdm(dataset):    
        if args.policy == 'aesthetic':
            score = float(item['aesthetic'])
            all_scores.append(score)
            label = score > args.aesthetic_score_cutoff                                               
            count[label] += 1  
            labels.append(label)      
        else:
...
def main(args):
    ...
    unet = UNet2DConditionModel.from_pretrained(
        args.ckpt or args.pretrained_model_name_or_path, subfolder="unet", revision=args.revision, variant=args.variant
    )
   
    #Aesthetic Validation
    aesthetic_predictor = None # ScorePredictor(device='cpu',metrics=args.eval_metrics)
    ...
    if args.use_ema:
        ema_unet = UNet2DConditionModel.from_pretrained(
             args.ckpt or args.pretrained_model_name_or_path, subfolder="unet", revision=args.revision
        )
    ...
    def preprocess_train(examples):
            ...
            all_pixel_values = []
            images = [Image.open(im_bytes).convert("RGB") for im_bytes in examples["jpg_0"]]
            original_sizes = [(image.height, image.width) for image in images]
            crop_top_lefts = []

            for col_name in ["jpg_0", "jpg_1"]:
                images = [Image.open(im_bytes).convert("RGB") for im_bytes in examples[col_name]]
                if col_name == "jpg_1":
            ...
            all_labels = []
            for im_tup, label_gt in zip(im_tup_iterator, examples["label"]):
                label_0 = (label_gt == 0) # first is better
                assert label_0
                if label_0 == 0:
                    im_tup = im_tup[::-1]
                ...
                all_labels.append(label_0) # not used
                # Cropping.
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                noisy_model_input = noise_scheduler.add_noise(latents, noise, timesteps).to(weight_dtype)
                ...
                if args.disable_sdxl:
                    ...
                    added_cond_kwargs={"time_ids": add_time_ids.to(weight_dtype), "text_embeds": pooled_prompt_embeds.to(weight_dtype)} 
                    model_pred = unet(
                        noisy_model_input.detach(),
                        timesteps,
                        prompt_embeds,
                ...
                labels = batch['labels']
                if args.dataloader == 'dpo':
                    labels = torch.cat([labels*0+1,labels*0])
                ...
                if args.bce_offset == "sigmoid":
                    ...
                    kl = (kl1* args.bce_alpha+kl2 * (1-args.bce_alpha))
                else:
                ...
                elif args.halo == 'sigmoid++':
                    h =  torch.sigmoid(label_scale_g)
                    with torch.no_grad():
                        scale_cls = (label_scale_g > 0).detach().float()
                        fac = scale_cls * args.h_pos + (1 -scale_cls ) * args.h_neg
                    h = h * fac.detach()
                ...
                w_y = args.lambda_d_tpo * labels_binary + args.lambda_u_tpo * ~labels_binary

                acc = ( label_scale_g > 0).sum().detach() / len(label_scale_g)
                n_pos = labels_binary.sum().item()
                n_neg = (len(labels_binary) - n_pos)
                if args.dataloader == 'dpo' and args.loss == 'dpo':
                    diff_pos = torch.tensor(0.0)
                    diff_neg = torch.tensor(0.0)
                    ref_losses_w, ref_losses_l = ref_loss.chunk(2)
                    ref_diff = ref_losses_w - ref_losses_l
                    raw_ref_loss = ref_loss.mean()
                ...
                #accelerator.backward(raw_model_loss)
                if args.loss == 'dpo':
                    loss = loss_dpo.mean()
                elif args.loss == 'sft':
                    loss = (model_losses * labels).mean() 
                elif args.loss == 'popalign':
                    loss_popalign = w_y * (-torch.nn.functional.logsigmoid(label_scale_g))
                    loss = loss_popalign.mean()   
                else:
                    raise NotImplemented
                    ...
                    if args.run_validation and global_step % args.validation_steps == 0 or global_step==1:
                        ...
                        if accelerator.is_main_process:
                            log_validation(
                                ...
                                epoch=epoch,aesthetic_predictor=aesthetic_predictor,global_step=global_step,
